...
RSpec.describe MagiMedicaid::PublishUqhpEligibleDocument do
  describe 'with valid arguments' do
    ...
    subject do
      described_class.new.call(family: family_hash, event_key: event_key)
    end
    ...
    context "when event key is missing" do
      ...
      let(:error) { "Missing event key #{family_hash[:hbx_id]}" }
    ...
    context "when event key is invalid" do
      let(:invalid_subject) { described_class.new.call(family: family_hash, event_key: invalid_event_key) }

      let(:invalid_event_key) { 'invalid_event_key' }

      let(:error) { "No template found for the given #{invalid_event_key}" }
    ...
    context "when input family hash in invalid" do
