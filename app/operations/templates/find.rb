...
module Templates
  # Search by key to find a {Templates::Template} in the database
  class Find
    ...
    def search(values)
      Try() do
        ...
        if values[:options].present?
          Templates::TemplateModel.send(scope_name, values[:options])
        else
          Templates::TemplateModel.send(scope_name)
        end
      end.bind do |result|
